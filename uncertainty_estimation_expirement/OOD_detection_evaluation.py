...
def uncertainty_threshold_testing(model, dataLoader, args):
    ...
    table = PrettyTable(['threshold', 'Number of filtered data points',  'Proportion','ACC'])
...
def run_OOD_Detection_expirement():
    ...
    Humam_model = torch.load('../results/Human_model.pth')

    print("-"*20,"Celegans->Celegans (In Distribution)","-"*20)
    uncertainty_threshold_testing(Celegans_model,test_Celegans_dataloader,args)

    print("-"*20,"Celegans->Human (Out of Distribution)","-"*20)
    uncertainty_threshold_testing(Celegans_model,test_Human_dataloader,args)

    print("-"*20,"Human->Human (In Distribution)","-"*20)
    uncertainty_threshold_testing(Humam_model,test_Human_dataloader,args)

    print("-" * 20, "Human->Celegans (Out of Distribution)", "-" * 20)
    uncertainty_threshold_testing(Humam_model, test_Celegans_dataloader, args)
